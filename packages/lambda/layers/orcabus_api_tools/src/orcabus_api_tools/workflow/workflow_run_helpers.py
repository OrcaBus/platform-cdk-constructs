...
from . import get_workflow_request, get_workflow_request_response_results
...
def get_workflow_run(workflow_run_orcabus_id: str) -> WorkflowRun:
    ...
    try:
        return get_workflow_request(
            endpoint=f"{WORKFLOW_RUN_ENDPOINT}/{workflow_run_orcabus_id}",
        )
...
def get_workflow_run_from_portal_run_id(portal_run_id: str) -> WorkflowRun:
    ...
    workflow_runs_list = get_workflow_request_response_results(
        ...
        return next(
            filter(
                ...
                get_workflow_request(
                    endpoint=f"{WORKFLOW_RUN_ENDPOINT}/{workflow_run_orcabus_id}/state",
                )
            )
        )
    except HTTPError as e:
