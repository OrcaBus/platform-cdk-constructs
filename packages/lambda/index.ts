            ...
            bundling: {
                ...
                commandHooks: {
                    ...
                    beforeBundling(inputDir: string, outputDir: string): string[] {
                        return props.bundling?.commandHooks?.beforeBundling(inputDir, outputDir) as unknown as string[] ?? [];
                    },
                    // @ts-ignore
                    afterBundling(inputDir: string, outputDir: string): string[] {
                        return (props.bundling?.commandHooks?.afterBundling(inputDir, outputDir) as unknown as string[] ?? []).concat([`rm -rf ${outputDir}/pandas/tests`]);
                    },
                },
            },
            paramsAndSecrets:
